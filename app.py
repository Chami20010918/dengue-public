        ...
        try:
            ...
            if not current_row.empty:
                ...
                if val > info["threshold"]:
                    status, color = "CRITICAL", [239, 68, 68, 180]
                elif val > info["threshold"] * 0.7:
                    status, color = "WARNING", [249, 115, 22, 180]
                else:
                    status, color = "NORMAL", [34, 197, 94, 180]
...
# --- 4. POPUP NOTIFICATION LOGIC ---
critical_zones = [d['name'] for d in dashboard_data if d['status'] == "CRITICAL"]
if critical_zones:
    st.toast(f"⚠️ OUTBREAK ALERT: {len(critical_zones)} Critical Zones Detected", icon="🚨")

# --- 5. HEADER WITH MOSQUITO ICON ---
st.markdown("""
<div class="header-container">
    ...
    <div style="color: #71717a; margin-top: 5px; font-weight: 500;">National AI-Driven Epidemic Surveillance Unit</div>
</div>
""", unsafe_allow_html=True)

if critical_zones:
    st.markdown(f"""<div class="emergency-banner">🚨 ALERT: Outbreak Levels Predicted in {', '.join(critical_zones)} for Feb 2026</div>""", unsafe_allow_html=True)
...
with k2: st.metric("Critical Zones", f"{high_risk_count}", delta=f"{high_risk_count}", delta_color="inverse")
with k3: st.metric("System Health", "STABLE", "Neural Core")
with k4: st.metric("Avg. Accuracy", "82.1%", "Verified")

st.markdown("---")

# --- 7. DARK THEMED MAP & DETAILS ---
    ...
    for city in dashboard_data:
        ...
        <div class="metric-card {is_crit}" style="margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between;">
                <span style="font-weight:800; font-size:1.1rem;">{city['name']}</span>
                <span class="badge {badge_type}">{city['status']}</span>
            </div>
            <div style="margin-top:10px; font-size:0.9rem;">
                Forecast: <b style="color:white;">{city['cases']} cases</b><br>
                Confidence: <span style="color:#22d3ee;">{city['acc']}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
